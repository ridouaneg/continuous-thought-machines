#!/bin/bash
#SBATCH --job-name=ctm_countix_cifar_rn18_2
#SBATCH -A kcn@h100
#SBATCH -C h100
#SBATCH --nodes=1
...
# Encoder ablation against train_countix_cifar_jz.sh:
# everything identical except backbone_type = resnet18-2 (the encoder
# the original failing Countix run used). Tells us whether the
# resnet18-1 vs resnet18-2 swap mattered, separate from the body
# shape change.
...
python -m tasks.repetition.train \
    ...
    --n_test_batches 30 \
    --num_workers_train 8 \
    --log_dir logs/repetition/countix_cifar_rn18_2 \
    --device 0 \
    --use_amp \
    --seed 42
